// config.js - Central de Configurações do seu Clã
module.exports = {
  // IDs dos emojis customizados do servidor usados nas tags (":arcolongo:", ":curandeiro:" etc)
  // dentro de `funcoes`. O index.js usa isso pra trocar a tag pelo ícone de verdade
  // (tanto no texto da mensagem quanto nos labels de botão/dropdown).
  emojisFuncao: {
  ...
  // Usado por calcularDificuldade() em index.js — atividades, zona e cidade
  // (ponto de encontro) entram todos na média.
  dificuldades_value: [
    // --- Zonas ---
  ...
  // Botões de bônus que aparecem no painel público. O jogador precisa já
  // estar em uma vaga pra poder marcar/desmarcar (o bot valida isso).
  // "emoji" é o unicode de verdade (o que aparece no botão e na linha da vaga);
  // "value" vira o customId do botão (bonus_mana, bonus_runa, etc).
  bonus: [
